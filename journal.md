...
Episode 5: Permutations, Transposes, Spaces R^n

This episode was going over permutations again, how many there are in a n x n, fulling having the elimation equation, that's what I'm naming PA=LU.
Then went on to transposes, where you swap each row for column, and when you multiply transposes you get a symmetric matrix. It is all quite simple, the way he puts it, and it is fun and easy.
Then he went into vector spaces and their subspaces. I feel that the way he taught it was the most simplest way of explaining it all, and if I explain it, I will be restating his lecture, but I will do it anyway.
R^n means all real vectors in with n components. This assumes column vectors, with that many components, and even includes matrices with equal rows. Now the real question is subspaces. Subspaces must follow the same rules that make up a vector space. You must be able to add all vectors, and multiply by any scalar. For R^2 that means all of R^2, a line in R^2, or the zero vector. For higher dimensions it means those shapes in between lines and the whole thing. For example in R^3, you can have a plane. To go beyond that there is no specific name, at least one that is not commonly used like lines or planes. When you take a matrix, you must be able to take all linear combinations of the columns in order to form a column subspace. That's all there is for a general idea.
I also just recently found out how the practice problems work. I should be able to complete problem set 1 and problem set 2 now that I am caught up. I had thought that there was 1 problem set per lecture but that is not true. They are split up along multiple lectures. I will try to do them again but I also want to get started on my project that will show my understanding of linear algebra further. I am going to think about it and not do it yet because I am still in school currently, hence the long break between lectures.

## 4-25-2026

Episode 6: Column Space and Nullspace

First a review on vector spaces and subspaces. To be a vector space you have v and w as vectors in a subspace. v + w must also be in the space and cv must also be in the space, where c is a scalar.
If you have 2 subspaces: P and L, then is P union L in the space? No, in this example where there P is a plane and L is a line that only meet at the origin. The space says that v+w and cv must stay in the space, where the space is the entire plane and the entire line. But is V is a vector along the line and P is a vector along the plane then your result can end up away from the line and the plane.
But P intersect L is a subspace because it is the zero vector. You can take the zero vector and add it to its self and stay on the zero vector. You can also multiply any c to the zero vector and remain the same making the zero vector a subspace.
Now the column space of a matrix is all linear combinations of the columns. For Ax = b, there is not a solution for every b. b is only a solution if it is in C(A) (column space of A.)
Now the nullspace of a matrix is all x's where Ax=0. This always be a subspace. Can also be written as a vector with constant c.
Simple, I guess. But these practice problems are still very difficult. Or maybe I am not used to looking at these type of problems. Probably both but I am not worried about getting the practice questions right, I just need to know the concepts.
